using StoryTeller.Core.ContributorAggregate.Events;
using StoryTeller.Core.Interfaces;

namespace StoryTeller.Core.ContributorAggregate.Handlers;
  ...
  IEmailSender emailSender) : INotificationHandler<ContributorDeletedEvent>
{
  public async Task Handle(ContributorDeletedEvent domainEvent, CancellationToken cancellationToken)
  {
    ...
    await emailSender.SendEmailAsync("to@test.com", "from@test.com", "Contributor Deleted", $"Contributor with id {domainEvent.ContributorId} was deleted.");
  }
}
